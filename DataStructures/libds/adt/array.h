...
namespace ds::adt {
    ...
    {
    public:
        // map function: (i,j) = (i - base1) * pocetStlpcov + (j - base2)
    ...
    T Array<T>::access(long long index) const
    {
        ...
        {
            throw std::out_of_range("Array<T>::access -> Invalid index!");
        }
    ...
    void Array<T>::set(T element, long long index)
    {
        ...
        if (!this->validateIndex(index))
        {
            throw std::out_of_range("Array<T>::set -> Invalid index!");
        }
    ...
    {
        // 08
        // implicitne z signed na unsigned pretyp na unsigned
        return index - base_;
    }
    ...
    T CompactMatrix<T>::access(long long index1, long long index2) const
    {
        ...
        {
            throw std::out_of_range("CompactMatrix<T>::access -> Invalid index!");
        }
    ...
    {
        // 08
        return index1 >= dimension1_.getBase() && index1 < dimension1_.getBase() + static_cast<long long>(dimension1_.getSize())
            && index2 >= dimension2_.getBase() && index2 < dimension2_.getBase() + static_cast<long long>(dimension2_.getSize());
    }
