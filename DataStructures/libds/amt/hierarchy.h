...
namespace ds::amt {
	...
	public:
		...
		virtual size_t nodeCount() const; //mohutnost celej hierarchie
	    virtual size_t nodeCount(const BlockType& node) const; //mohutnost podhierarchie
    ...
    void BinaryHierarchy<BlockType>::processInOrder(const BlockType* node, std::function<void(const BlockType*)> operation) const
	{
		...
		if (node != nullptr)
		{
			BlockType* leftSon = this->accessLeftSon(*node);
			BlockType* rightSon = this->accessRightSon(*node);
			if (leftSon != nullptr)
			{
