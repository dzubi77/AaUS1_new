...
namespace ds::amt {
		...
		public ExplicitHierarchyBlock<DataType>
	{
		...
		IS<KWayExplicitHierarchyBlock<DataType, K>*>* sons_; // pole :D
	};
	...
	auto MultiWayExplicitHierarchy<DataType>::emplaceSon(BlockType& parent, size_t sonOrder) -> BlockType&
	{
		...
		newSon->parent_ = &parent; //nezabudnut priradit synovi otca :D
		return *newSon;
	}
	...
	void MultiWayExplicitHierarchy<DataType>::changeSon(BlockType& parent, size_t sonOrder, BlockType* newSon)
	{
		...
		if (oldSon != nullptr)
		{
			oldSon->parent_ = nullptr; //pretrhnutie vazby medzi otcom a starym synom
		}
		if (newSon != nullptr)
		{
